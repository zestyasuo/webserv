...
HTTPRequest::HTTPRequest(std::string const &raw)
	: AHTTPMessage(raw), valid(false), target(""), method("")
{
	std::vector< std::string > meta_data = get_meta_data();
	if (meta_data.empty())
		return;
	std::string				   status_line = meta_data[0];
	std::vector< std::string > status_line_vec =
		split_if(status_line, ::isspace);
...
HTTPRequest::parse_headers(std::vector< std::string > const &meta) const
{
	std::string							 first;
	std::string							 second;
	std::map< std::string, std::string > res;

	for (std::vector< std::string >::const_iterator it = meta.begin() + 1;
		 it != meta.end(); it++)
	{
...
std::string
HTTPRequest::parse_method(std::vector< std::string > const &status_line) const
{
	return status_line[METHOD];
}

std::string
HTTPRequest::parse_target(std::vector< std::string > const &status_line) const
{
	return status_line[TARGET];
}

std::string
HTTPRequest::parse_version(std::vector< std::string > const &status_line) const
{
